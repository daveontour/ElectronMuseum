...
const Chat = (() => {
    ...
    /** Embedded JSON key messageInStars (fenced json block, same mechanism as response action buttons) updates the starfield canvas. */
    function _applyMessageInStarsFromEmbedded(embeddedJson) {
        if (!embeddedJson || typeof window.ChatMessageStars === 'undefined') return;
        if (!Object.prototype.hasOwnProperty.call(embeddedJson, 'messageInStars')) return;
        const v = embeddedJson.messageInStars;
        if (v === null) {
            window.ChatMessageStars.clearOverride();
        } else if (typeof v === 'string') {
            window.ChatMessageStars.setOverride(v);
        }
    }

    function _plainTextForTTS(contentElement) {
        return (contentElement.textContent || '').replace(/\s+/g, ' ').trim();
    }

    function _ensureSpeechVoices() {
    ...
    function _pickEnglishSpeechVoice(voices) {
        if (!voices || !voices.length) return null;
        const enUS = voices.find((v) => v.lang && /^en-us$/i.test(v.lang.trim()));
        if (enUS) return enUS;
        const en = voices.find((v) => v.lang && v.lang.toLowerCase().startsWith('en'));
        return en || voices[0];
    }

    const SPEAK_ICON_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M14,3.23V5.29C16.89,6.15 19,8.83 19,12C19,15.17 16.89,17.84 14,18.7V20.77C18,19.86 21,16.28 21,12C21,7.72 18,4.14 14,3.23M16.5,12C16.5,10.23 15.5,8.71 14,7.97V16C15.5,15.29 16.5,13.76 16.5,12M3,9V15H7L12,20V4L7,9H3Z"/></svg>';
    const PAUSE_ICON_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>';
    ...
    function _addSpeakButton(contentElement, role) {
        ...
        speakButton.setAttribute('aria-label', 'Read response aloud');
        speakButton.title = 'Read response aloud';
        speakButton.innerHTML = SPEAK_ICON_SVG;

        const statusMessage = document.createElement('div');
        statusMessage.className = 'speak-status';
        ...
        const setSpeakIdle = () => {
            speakButton.classList.remove('speak-button--playing');
            speakButton.setAttribute('aria-label', 'Read response aloud');
            speakButton.title = 'Read response aloud';
            speakButton.innerHTML = SPEAK_ICON_SVG;
        };

        const setSpeakPlaying = () => {
            speakButton.classList.add('speak-button--playing');
            speakButton.setAttribute('aria-label', 'Stop speaking');
            speakButton.title = 'Stop speaking';
            speakButton.innerHTML = PAUSE_ICON_SVG;
        };

        speakButton.onclick = () => {
            if (!window.speechSynthesis) {
                showAudioStatus('Speech synthesis is not available in this browser.', true);
                return;
            }

            if (speakButton.classList.contains('speak-button--playing')) {
            ...
            const text = _plainTextForTTS(contentElement);
            if (!text) {
                showAudioStatus('Nothing to read.', true);
                return;
            }
            ...
            void (async () => {
                try {
                    showAudioStatus('Preparing voice…');
                    await _ensureSpeechVoices();
                    const voices = window.speechSynthesis.getVoices();
                    const utterance = new SpeechSynthesisUtterance(text);
                    const preferred = _pickEnglishSpeechVoice(voices);
                    if (preferred) utterance.voice = preferred;
                    utterance.lang = preferred && preferred.lang ? preferred.lang : 'en-US';

                    utterance.onstart = () => {
                        setSpeakPlaying();
                        showAudioStatus('Speaking…');
                    };
                    utterance.onend = () => {
                        setSpeakIdle();
                        statusMessage.style.display = 'none';
                    ...
                    utterance.onerror = (e) => {
                        ...
                        showAudioStatus(e.error || 'Speech error', true);
                    };
                    ...
                    setSpeakIdle();
                    showAudioStatus(error.message || 'Could not read aloud', true);
                }
            })();
        };
    }
    ...
    function clearChat() {
        ...
        DOM.chatBox.querySelectorAll(
            ':scope > .message, :scope > .have-a-chat-message, :scope > .have-a-chat-user-comment, :scope > .interview-question, :scope > .interview-answer, :scope > .interview-loading, :scope > .interview-writeup'
        ).forEach((el) => el.remove());
        if (typeof window.ChatMessageStars !== 'undefined' && window.ChatMessageStars.clearOverride) {
            window.ChatMessageStars.clearOverride();
        }
        document.querySelector('.chat-box-wrapper')?.classList.remove('chat-pane-messages-hidden');
        const visToggle = document.getElementById('chat-messages-visibility-toggle');
        if (visToggle) {
            visToggle.setAttribute('aria-pressed', 'false');
            visToggle.title = 'Hide messages to see background';
            visToggle.setAttribute('aria-label', 'Hide chat messages');
            const ic = visToggle.querySelector('i');
...
const ApiService = (() => {
    ...
    return {
        fetchChat, fetchRandomQuestion, fetchNewChat, fetchSuggestionsConfig,
