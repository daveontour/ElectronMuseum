...
func (p *ClaudeProvider) SimpleGenerate(ctx context.Context, prompt string) (string, *LLMUsage, error) {
	if p == nil || p.apiKey == "" {
	...
	usage := claudeUsageFromResponse(resp, p.modelName)
...
func claudeUsageFromResponse(resp map[string]any, modelName string) *LLMUsage {
	in, out := 0, 0
	...
	return &LLMUsage{Provider: "claude", Model: modelName, InputTokens: in, OutputTokens: out}
}
	...
	for iter := 0; iter < maxToolCallIterations; iter++ {
		...
		if stopReason != "tool_use" {
			...
			return GenerateResult{
				...
				Voice:        req.Voice,
				Usage: &LLMUsage{
					Provider: "claude", Model: p.modelName,
		...
		for _, item := range contentRaw {
			...
			if toolName == "get_reference_document" || toolName == "get_sensitive_reference_document" {
