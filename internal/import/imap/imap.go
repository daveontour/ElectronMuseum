...
func normalizeIMAPHostSource(host string) string {
...
func ImportFolders(ctx context.Context, pool *sql.DB, p ConnParams, folders []string, newOnly bool, cancelFn func() bool, progressFn ProgressFunc) (int, error) {
	...
	mailSource := normalizeIMAPHostSource(p.Host)
	totalProcessed := 0
	for idx, folder := range folders {
		...
		count, err := importFolder(ctx, c, pool, folder, newOnly, cancelFn, mailSource)
		if err != nil {
